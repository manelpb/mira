...
class ReviewEngine:
    ...
    async def _review_diff_internal(
        ...
        async def _review_chunk(
            ...
            async with semaphore:
                ...
                try:
                    ...
                    # Also save comments so the bucketing loop can recover them
                    # if re-parsing fails (a failure there silently drops them).
                    key_issues: list[KeyIssue] = []
                    summary_text = ""
                    first_run_comments: list[ReviewComment] | None = None
                    try:
                        first_run_comments, key_issues, summary_text = _parse(raw_response)
                    ...
                    for (role, _), result in zip(calls, results, strict=True):
                        if isinstance(result, BaseException):
                            ...
                            continue
                        if role == "primary" and first_run_comments is not None:
                            # Use the pre-parsed comments — re-parsing the
                            # same raw_response risks dropping them on error.
                            cs = first_run_comments
                        else:
                            try:
        ...
        indexing_cost = 0
        if self.indexing_llm is not None and self.indexing_llm is not self.llm:
